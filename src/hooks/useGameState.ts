...
function gameReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    ...
    case "EXTEND_PATH": {
      ...
      // Must not be a wall
      if (state.level.walls.some(w => w.row === position.row && w.col === position.col)) {
        return state;
      }
      ...
      const cols = state.level.gridCols ?? state.level.gridSize;
      const isComplete = checkWin(
        newPath,
        state.level.gridSize,
        cols,
        state.level.walls.length,
    ...
    case "SELECT_LEVEL": {
      // Level is pre-generated by the caller (with uid baked into the seed)
